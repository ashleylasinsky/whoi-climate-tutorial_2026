...
### Goal for tutorial 
The goal for today is to become familiar with how to load, preprocess, and analyze (e.g., compute statistics on) gridded climate data. In particular, we'll define a climate "index" and compute some statistics on the index over the time period covered in the dataset. In the example, we analyze sea surface temperature (SST) data from a widely-used reanalysis product, ERA5 (Hersbach et al, 2020), defining an index for the region around Woods Hole. After looking through this example, you will modify it for a dataset and index of your choice.

#### To-do list
1. **Choose a datset**: To stick most closely to the example, you could use SST from ERA5. You could also choose a different variable from the ERA5 dataset, e.g. 2-meter atmospheric temperature, sea level pressure, precipitation, etc. The final option is to use the dataset you downloaded in advance of class.
2. **Define a climate index**: choose a region (e.g., the North Atlantic Ocean) and a variable (e.g., sea surface temperature), then define an "index" which can be evaluated at every timestep (e.g., SST averaged over the North Atlantic).
3. **Preprocess the climate index**: compute "anomalies" by removing the time-mean seasonal cycle.
4. **Plot some diagnostics**: try to characterize variability of the index. Here are a few ideas:
